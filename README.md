...
The preview page is a mock chat UI plus a 96-theme gallery with live contrast badges. It loads `preview/themes.json` over `fetch()`, so it **must** be served over HTTP — under `file://` the browser blocks the request and the page reports that it cannot load `themes.json`.
...
Screenshots of that page: `preview/screenshot-light.png`, `preview/screenshot-dark.png`.
...
| `preview/` | Standalone gallery (`themes.json` is the same batch as the generated roster) |
...
- **No screenshots from a live harness yet.** On-device verification was done by reading computed styles (browser screenshots were unavailable in that environment); the images in `preview/` are of the standalone preview page.
